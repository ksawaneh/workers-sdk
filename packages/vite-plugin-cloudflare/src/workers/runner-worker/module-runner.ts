...
declare global {
	var __VITE_ENVIRONMENT_RUNNER_IMPORT__:
		| ((environmentName: string, id: string) => Promise<unknown>)
		| undefined;
}
...
/** The parent environment name (set explicitly via IS_WORKER_ENVIRONMENT_HEADER) */
...
export class __VITE_RUNNER_OBJECT__ extends DurableObject<WrapperEnv> {
	...
	override async fetch(request: Request) {
		...
		if (isParentEnvironment) {
			if (parentEnvironmentName !== undefined) {
				throw new Error(
					`Parent environment already set to: ${parentEnvironmentName}`
				);
			}
